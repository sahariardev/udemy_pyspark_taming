...
customer_purchase=data.map(extractCustomerIdAndPurchaseAmount).reduceByKey(lambda x,y:x+y).map(exchange_key_value_order).sortByKey()
results=customer_purchase.collect()

for result in results:
    print("User id : {a} Amount Spent: {b:.2f} USD ".format(a=result[1],b=result[0]))
